...
void Video::Init()
{
    ...
    m_pSpriteXCacheBuffer = new u8[GAMEBOY_WIDTH * GAMEBOY_HEIGHT];
...
void Video::SaveState(std::ostream& stream)
{
    ...
    stream.write(reinterpret_cast<const char*> (m_pSpriteXCacheBuffer), GAMEBOY_WIDTH * GAMEBOY_HEIGHT);
...
void Video::LoadState(std::istream& stream, u32 version)
{
    ...
    stream.read(reinterpret_cast<char*> (m_pSpriteXCacheBuffer), GAMEBOY_WIDTH * GAMEBOY_HEIGHT);
