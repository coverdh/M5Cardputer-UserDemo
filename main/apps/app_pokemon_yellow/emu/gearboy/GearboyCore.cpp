...
void GearboyCore::Init(GB_Color_Format pixelFormat)
{
...
#if !defined(GEARBOY_ESP32_NO_SGB)
    m_pSGB = new SGB(m_pMemory, m_pVideo);
#endif
#if !defined(GEARBOY_ESP32_NO_SGB_BUFFER)
    ...
    InitMemoryRules();
    InitDMGPalette();
#if !defined(GEARBOY_DISABLE_COLOR_CORRECTION)
...
bool GearboyCore::LoadROMFromExternalBuffer(const u8* buffer, int size, const char* path, bool forceDMG, Cartridge::CartridgeTypes forceType, bool forceGBA)
{
    if (m_pCartridge->LoadFromExternalBuffer(buffer, size, path))
    {
        m_bForceDMG = forceDMG;
        Reset(m_bForceDMG ? false : m_pCartridge->IsCGB(), forceGBA);
        m_pMemory->ResetDisassemblerRecords();
