...
bool Cartridge::LoadFromExternalBuffer(const u8* buffer, int size, const char* path)
{
    if (IsValidPointer(buffer))
    {
        Reset();
        Log("Loading from external buffer... Size: %d", size);
        m_iTotalSize = size;
        m_pTheROM = const_cast<u8*>(buffer);
        m_bExternalROM = true;
        m_bLoaded = true;

        if (IsValidPointer(path))
        {
            strncpy(m_szFilePath, path, sizeof(m_szFilePath) - 1);
            m_szFilePath[sizeof(m_szFilePath) - 1] = 0;

            const char* slash = strrchr(path, '/');
            const char* file_name = slash ? slash + 1 : path;
            strncpy(m_szFileName, file_name, sizeof(m_szFileName) - 1);
            m_szFileName[sizeof(m_szFileName) - 1] = 0;

            if (slash)
            {
                size_t dir_len = static_cast<size_t>(slash - path);
                if (dir_len >= sizeof(m_szFileDirectory))
                    dir_len = sizeof(m_szFileDirectory) - 1;
                memcpy(m_szFileDirectory, path, dir_len);
                m_szFileDirectory[dir_len] = 0;
            }
        }
