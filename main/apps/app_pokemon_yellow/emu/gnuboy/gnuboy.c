...
static byte *external_sram_buffer;
static size_t external_sram_length;
static bool cart_uses_external_sram;
...
void gnuboy_set_sram_buffer(void *buffer, size_t length)
{
	external_sram_buffer = (byte *)buffer;
	external_sram_length = length;
}


void gnuboy_clear_sram_buffer(void)
{
	external_sram_buffer = NULL;
	external_sram_length = 0;
}
...
int gnuboy_load_rom(const byte *data, size_t size)
{
	...
	size_t sram_length = cart.ramsize * 0x2000;
	if (external_sram_buffer && external_sram_length >= sram_length)
	{
		cart.rambanks = (byte (*)[8192])external_sram_buffer;
		cart_uses_external_sram = true;
		memset(cart.rambanks, 0xff, sram_length);
	}
	else
	{
		cart.rambanks = calloc(cart.ramsize, 0x2000);
		cart_uses_external_sram = false;
	}
...
int gnuboy_save_sram(const char *file, bool quick_save)
{
	...
	FILE *f = fopen(file, quick_save ? "r+b" : "wb");
	if (!f && quick_save)
	{
		quick_save = false;
		f = fopen(file, "wb");
