...
static byte *hw_ram_storage[8];
static byte *hw_vram_storage[2];
...
bool gb_hw_init(void)
{
	...
	{
		if (!hw_ram_storage[i])
			hw_ram_storage[i] = calloc(1, 4096);
		hw.rambanks[i] = hw_ram_storage[i];
	}
	for (int i = 0; i < 2; ++i)
	{
		if (!hw_vram_storage[i])
			hw_vram_storage[i] = calloc(1, 8192);
		hw.vbanks[i] = hw_vram_storage[i];
	}
...
void gb_hw_reset(bool hard)
{
	...
	{
		for (int i = 0; i < 8; ++i)
			memset(hw.rambanks[i], 0xff, 4096);
		if (cart.rambanks)
